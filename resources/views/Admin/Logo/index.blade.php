            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header align-items-center d-flex">
                        <h5 class="card-title mb-0 flex-grow-1">List all Logo</h5>
                        <div class="flex-shrink-0">
                            <a href="{{route('admin.logo.add')}}" class="btn btn-primary btn-label"><i class=" ri-add-line label-icon align-middle fs-16 me-2"></i> Add Logo</a>
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        <table id="logoTable" class="display table table-bordered dt-responsive dataTable dtr-inline" style="width:100%">
                            <thead>
...
<script>
    $(function(){
      'use strict'
         var oTable = $('#logoTable').DataTable({
            "processing": true,
             "bServerSide": true,
             "sAjaxSource": "{{ route('admin.logo.ajax-list') }}", 
             "aaSorting": [ [5, "desc"] ],  
    ...
    $(document).on('click','.logo_remove', function(){
            var idStr = $(this).data('id');
            Swal.fire({
                title: "Are you sure?",
                text: "This Logo is not display after this action!",
                icon: "warning",
                showCancelButton: !0,
        ...
        $(document).on('click','.logo_active', function(){
            var idStr = $(this).data('id');
            Swal.fire({
                title: "Are you sure?",
                text: "This logo not able to login after this action!",
                icon: "warning",
                showCancelButton: !0,
                confirmButtonClass: "btn btn-primary w-xs me-2 mt-2",
                cancelButtonClass: "btn btn-danger w-xs mt-2",
        ...
        $(document).on('click','.logo_inactive', function(){
            var idStr = $(this).data('id');
            Swal.fire({
                title: "Are you sure?",
                text: "This logo able to login after this action!",
                icon: "warning",
                showCancelButton: !0,
                confirmButtonClass: "btn btn-primary w-xs me-2 mt-2",
                cancelButtonClass: "btn btn-danger w-xs mt-2",
    ...
    function statusChange(oTable, id, val)
    {
        ...
        setTimeout(() => {
            $.ajax({
                url: "{{ route('admin.logo.update-status') }}",
    ...
    function removeLogo(oTable, id)
    {
        $('.vertical-overlay').show();
        setTimeout(() => {
            $.ajax({
                url: "{{ route('admin.logo.remove') }}",
