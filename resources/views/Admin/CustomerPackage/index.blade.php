            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header align-items-center d-flex">
                        <h5 class="card-title mb-0 flex-grow-1">List all Customer</h5>
                        <div class="flex-shrink-0">
                            <a href="{{route('admin.customer.packages.add')}}" class="btn btn-primary btn-label"><i class=" ri-add-line label-icon align-middle fs-16 me-2"></i> Add Customer</a>
                        </div>
                    </div>
                    <div class="card-body">
...
<script>
    $(function(){
      'use strict'
         var oTable = $('#customerTable').DataTable({
            "processing": true,
             "bServerSide": true,
             "sAjaxSource": "{{ route('admin.customer.packages.ajax-list') }}", 
             "aaSorting": [ [5, "desc"] ],  
    ...
    $(document).on('click','.customer_remove', function(){
            ...
            Swal.fire({
                title: "Are you sure?",
                text: "This Customer is not display after this action!",
                icon: "warning",
                showCancelButton: !0,
        ...
        $(document).on('click','.customer_active', function(){
            ...
            Swal.fire({
                title: "Are you sure?",
                text: "This customer not able to login after this action!",
                icon: "warning",
                showCancelButton: !0,
                confirmButtonClass: "btn btn-primary w-xs me-2 mt-2",
                cancelButtonClass: "btn btn-danger w-xs mt-2",
        ...
        $(document).on('click','.customer_inactive', function(){
            ...
            Swal.fire({
                title: "Are you sure?",
                text: "This customer able to login after this action!",
                icon: "warning",
                showCancelButton: !0,
                confirmButtonClass: "btn btn-primary w-xs me-2 mt-2",
                cancelButtonClass: "btn btn-danger w-xs mt-2",
    ...
    function statusChange(oTable, id, val)
    {
        ...
        setTimeout(() => {
            $.ajax({
                url: "{{ route('admin.customer.packages.update-status') }}",
    ...
    function removeCustomer(oTable, id)
    {
        $('.vertical-overlay').show();
        setTimeout(() => {
            $.ajax({
                url: "{{ route('admin.customer.packages.remove') }}",
