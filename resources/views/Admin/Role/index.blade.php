            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header align-items-center d-flex">
                        <h5 class="card-title mb-0 flex-grow-1">List all roles</h5>
                        <div class="flex-shrink-0">
                            <a href="{{route('admin.roles.add')}}" class="btn btn-primary btn-label"><i class=" ri-add-line label-icon align-middle fs-16 me-2"></i> Add Role</a>
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        <table id="roleTable" class="display table table-bordered dt-responsive dataTable dtr-inline" style="width:100%">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Name</th>
...
<script>
    $(function(){
      'use strict'
        var oTable = $('#roleTable').DataTable({
            "processing": true,
            "bServerSide": true,
            "sAjaxSource": "{{ route('admin.roles.ajax-list') }}",
            "aaSorting": [ [3, "desc"] ],
            ...
            'aoColumns': [
                ...
                {"sWidth": "20%"},
                {"sWidth": "5%", 'bSortable': false, "class": 'text-center'}
            ],
            language: {
        ...
        $(document).on('click','.role_active', function(){
            var idStr = $(this).data('id');
            Swal.fire({
                title: "Are you sure?",
                text: "This role's user not able to login after this action!",
                icon: "warning",
                showCancelButton: !0,
        ...
        $(document).on('click','.role_inactive', function(){
            var idStr = $(this).data('id');
            Swal.fire({
                title: "Are you sure?",
                text: "This role's user able to login after this action!",
                icon: "warning",
                showCancelButton: !0,
                confirmButtonClass: "btn btn-primary w-xs me-2 mt-2",
                cancelButtonClass: "btn btn-danger w-xs mt-2",
        ...
        $(document).on('click','.role_remove', function(){
            ...
            Swal.fire({
                title: "Are you sure?",
                text: "This role is not display after this action!",
                icon: "warning",
                showCancelButton: !0,
                confirmButtonClass: "btn btn-primary w-xs me-2 mt-2",
                cancelButtonClass: "btn btn-danger w-xs mt-2",
    ...
    function removeRole(oTable, id)
    {
        $('.vertical-overlay').show();
        setTimeout(() => {
            $.ajax({
                url: "{{ route('admin.roles.remove') }}",
