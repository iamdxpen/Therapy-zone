            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header align-items-center d-flex">
                        <h5 class="card-title mb-0 flex-grow-1">List all Gallery</h5>
                        <div class="flex-shrink-0">
                            <a href="{{route('admin.gallery.add')}}" class="btn btn-primary btn-label"><i class=" ri-add-line label-icon align-middle fs-16 me-2"></i> Add Gallery</a>
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        <table id="galleryTable" class="display table table-bordered dt-responsive dataTable dtr-inline" style="width:100%">
                            <thead>
...
<script>
    $(function(){
      'use strict'
         var oTable = $('#galleryTable').DataTable({
            "processing": true,
             "bServerSide": true,
             "sAjaxSource": "{{ route('admin.gallery.ajax-list') }}", 
             "aaSorting": [ [5, "desc"] ],  
    ...
    $(document).on('click','.gallery_remove', function(){
            ...
            Swal.fire({
                title: "Are you sure?",
                text: "This Gallery is not display after this action!",
                icon: "warning",
                showCancelButton: !0,
        ...
        $(document).on('click','.gallery_active', function(){
            var idStr = $(this).data('id');
            Swal.fire({
                title: "Are you sure?",
                text: "This gallery not able to login after this action!",
                icon: "warning",
                showCancelButton: !0,
                confirmButtonClass: "btn btn-primary w-xs me-2 mt-2",
                cancelButtonClass: "btn btn-danger w-xs mt-2",
        ...
        $(document).on('click','.gallery_inactive', function(){
            var idStr = $(this).data('id');
            Swal.fire({
                title: "Are you sure?",
                text: "This gallery able to login after this action!",
                icon: "warning",
                showCancelButton: !0,
                confirmButtonClass: "btn btn-primary w-xs me-2 mt-2",
                cancelButtonClass: "btn btn-danger w-xs mt-2",
    ...
    function statusChange(oTable, id, val)
    {
        ...
        setTimeout(() => {
            $.ajax({
                url: "{{ route('admin.gallery.update-status') }}",
    ...
    function removeGallery(oTable, id)
    {
        $('.vertical-overlay').show();
        setTimeout(() => {
            $.ajax({
